...
if [ "$resume_step" != "New_run" ]; then
	...
	if [ "$resume_step" == "variant" ]; then
		...
		rm -r $OUT_DIR/align
		rm -r $OUT_DIR/tree
		rm -r $OUT_DIR/Logs/${resume_step}
		rm -r $OUT_DIR/Logs/extract
		rm -r $OUT_DIR/Logs/align
		rm -r $OUT_DIR/Logs/tree
		rm -r $OUT_DIR/Logs/${resume_step}.sh
		rm -r $OUT_DIR/Logs/extract.sh
		rm -r $OUT_DIR/Logs/align.sh
		rm -r $OUT_DIR/Logs/tree.sh
		rm -r $OUT_DIR/Logs/Extraction_pipeline.sh
		rm -r $OUT_DIR/Logs/Extraction_pipeline.err
		rm -r $OUT_DIR/Logs/Extraction_pipeline.out
...
if [[ "$resume_step" != "variant" && "$resume_step" != "extract" && "$resume_step" != "align" && "$resume_step" != "tree" ]]; then
	...
	mkdir $OUT_DIR/coverage
	mkdir $OUT_DIR/Logs/coverage
	mkdir $OUT_DIR/Logs/coverage/Failed
...
if [[ "$resume_step" != "extract" && "$resume_step" != "align" && "$resume_step" != "tree" ]]; then
	...
	# Create necessary directories
	mkdir $OUT_DIR/variant
	mkdir $OUT_DIR/Logs/variant

	ncol=$(awk -v 'FS=\t' '{print NF}' $SAMPLES | sort -nu | tail -n 1)
	if [[ "$ncol" -eq 3 ]]; then
		cat $SAMPLES | awk -v 'FS=\t' -v 'OFS=\t' '{ print $1, $2}' | sed 's/ /_/g' | sort | uniq > $OUT_DIR/groups.txt
	fi
...
if [[ "$resume_step" != "align" && "$resume_step" != "tree" ]]; then
	...
	mkdir $OUT_DIR/extract
	mkdir $OUT_DIR/Logs/extract
...
if [[ "$resume_step" != "tree" ]]; then
	...
	# Create output directory
	mkdir $OUT_DIR/align
	mkdir $OUT_DIR/Logs/align
	mkdir $OUT_DIR/Logs/align/Failed
...
# Create output directory
mkdir $OUT_DIR/tree
mkdir $OUT_DIR/Logs/tree
mkdir $OUT_DIR/tree/Other

sbatch --export=ALL --array=1-${gene_count} --output=$DEF_DIR/Logs/%x.%A/%x.%j.out --error=$DEF_DIR/Logs/%x.%A/%x.%j.err $DEF_DIR/tree.sh
